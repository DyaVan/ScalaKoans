...
class AboutMaps extends KoanSuite {
  ...
  koan("Maps contain distinct pairings") {
    val myMap = Map("MI" -> "Michigan", "OH" -> "Ohio", "WI" -> "Wisconsin", "MI" -> "Michigaon")
  ...
  koan("Map values can be iterated") {
    ...
    mapValues.toList.contains("Illinois") should be (false)
  }
...
//    //}
  ...
  koan("Map elements can be removed in multiple") {
    var myMap = Map("MI" -> "Michigan", "OH" -> "Ohio", "WI" -> "Wisconsin", "IA" -> "Iowa")
