...
class AboutApply extends KoanSuite {
  ...
  koan("""The apply method can also be used in singleton objects as well, in fact, it is the most common way
    ...
    object Employee {
        def apply(firstName:String, lastName:String) = new Employee(lastName,firstName)
    }

    var a = Employee("Aleksander", "Neufied")
    a.firstName should be ("Neufied")
    val b = new Employee("Jamie", "Pindar")
